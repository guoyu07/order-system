...
@Service
@Transactional
public class CustCancelOutImpl extends AbstractCancelTemplate {
	ExceptionHandler exceptionHandler = new ExceptionHandler(LogFactory.getLog(CustCancelOutImpl.class));
	...
	public void throwEvent(int orderId) {
		eventService.throwCustCancelOrderEvent(orderId);
	}

	@Override
	public void operCouponCard(int orderId) {
		exceptionHandler.getLog().info("用户取消订单,可能需要取消卡券,orderId:" + orderId);
