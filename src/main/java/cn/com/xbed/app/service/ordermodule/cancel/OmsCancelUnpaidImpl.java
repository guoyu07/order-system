...
@Service
@Transactional
public class OmsCancelUnpaidImpl extends AbstractCancelTemplate {
	ExceptionHandler exceptionHandler = new ExceptionHandler(LogFactory.getLog(OmsCancelUnpaidImpl.class));
	...
	public void throwEvent(int orderId) {
		eventService.throwOMSCancelUnpaidOrderEvent(orderId);
	}

	@Override
	public void operCouponCard(int orderId) {
		exceptionHandler.getLog().info("OMS消未支付订单,可能需要取消卡券,orderId:" + orderId);
